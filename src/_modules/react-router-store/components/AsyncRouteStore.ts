...
export class AsyncRouteStore implements RouteStore {
  ...
  preload = (path: string) => {
    return new Promise<void>((resolve: () => void, reject: (error: Error) => void) => {
      const config: AsyncRoute | undefined = this.routes.find(({component, ...routeProps}: AsyncRoute) => {
        return matchPath(path, {...routeProps}) !== null;
      });
      
      if (config) {
        config.component().then(({default: Component}: {default: ComponentType<any>}) => {
  ...
  getRoute = (path: string) => {
    const route: AsyncRoute | undefined = this.routes.find((r: AsyncRoute) => r.path === path);
  ...
  getAllRoutes = () => {
    return this.routes.map(({path}: AsyncRoute) => this.getRoute(path));
  };
  
  getRouteOptions = () => {
    return this.routes.map(({component, ...routeOption}: AsyncRoute) => routeOption);
  };
}